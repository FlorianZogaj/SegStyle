...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std,
    ...
    for layer in cnn.children():
        ...
        if name in style_layers:
            ...
            resized_mask = cv2.resize(masks.astype(np.float32), (target_feature.shape[3], target_feature.shape[2]), interpolation=cv2.INTER_NEAREST)
...
class StyleTransfer:
    ...
    def __call__(self, img, masks_obj, mask_background, style_obj, style_background, num_steps=300):
        # TODO take from tutorial
        loader = transforms.Compose([
            transforms.Resize((563, 1000)),  # scale imported image
            transforms.ToTensor()]
        )  # transform it into a torch tensor

        content_tensor = loader(Image.fromarray(img)).unsqueeze(0).to(self.device)
        style_tensors_obj = loader(style_obj).unsqueeze(0).to(self.device)
        ...
        output_tensor_obj = run_style_transfer(
            ...
            content_tensor, style_tensors_obj, masks_obj, input_tensor,
            num_steps=num_steps
        )
